      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <span className="mx-2 text-slate-300 dark:text-neutral-800">•</span>
            <span className="text-xs font-bold text-slate-500 dark:text-neutral-400 uppercase tracking-wider">
          ...
          <button
            type="button"
            onClick={onClose}
            className="flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 dark:border-neutral-850 text-slate-400 dark:text-neutral-400 bg-slate-50 dark:bg-neutral-900/40 transition hover:text-slate-700 dark:hover:text-white hover:border-slate-300 dark:hover:border-neutral-700 hover:bg-slate-100 dark:hover:bg-neutral-900"
        ...
        <div className="relative mb-5 rounded-xl border border-slate-100 dark:border-neutral-900 bg-slate-50/50 dark:bg-neutral-900/40 p-4 group">
          ...
          {images.length > 1 && (
            <>
              <button
                ...
                className="absolute left-2 top-1/2 -translate-y-1/2 flex h-8 w-8 items-center justify-center rounded-full bg-white/90 dark:bg-neutral-900/90 border border-slate-200 dark:border-neutral-800 text-slate-600 dark:text-neutral-300 hover:text-slate-900 dark:hover:text-white shadow hover:bg-slate-50 dark:hover:bg-neutral-850 transition"
              >
              ...
              <button
                ...
                className="absolute right-2 top-1/2 -translate-y-1/2 flex h-8 w-8 items-center justify-center rounded-full bg-white/90 dark:bg-neutral-900/90 border border-slate-200 dark:border-neutral-800 text-slate-600 dark:text-neutral-300 hover:text-slate-900 dark:hover:text-white shadow hover:bg-slate-50 dark:hover:bg-neutral-850 transition"
              >
                  ...
                  <button
                    ...
                    className={`h-11 w-11 rounded-lg border overflow-hidden bg-white dark:bg-neutral-950 p-1 transition ${
                      activeImageIndex === idx
                        ...
                        : 'border-slate-200 dark:border-neutral-850 opacity-60 hover:opacity-100'
                    }`}
                  >
        ...
        <div className="mb-6">
          ...
          <div className="rounded-lg border border-slate-100 dark:border-neutral-900/80 overflow-hidden text-xs bg-slate-50 dark:bg-neutral-950">
            <div className="divide-y divide-slate-100 dark:divide-neutral-900">
              {loadingDetails ? (
                <div className="space-y-3.5 p-4">
                  <div className="flex justify-between items-center animate-pulse">
                    <div className="h-3.5 w-24 bg-slate-200 dark:bg-neutral-800 rounded"></div>
                    <div className="h-3.5 w-16 bg-slate-200 dark:bg-neutral-800 rounded"></div>
                  </div>
                  <div className="flex justify-between items-center animate-pulse">
                    <div className="h-3.5 w-20 bg-slate-200 dark:bg-neutral-800 rounded"></div>
                    <div className="h-3.5 w-12 bg-slate-200 dark:bg-neutral-800 rounded"></div>
                  </div>
                  <div className="flex justify-between items-center animate-pulse">
                    <div className="h-3.5 w-28 bg-slate-200 dark:bg-neutral-800 rounded"></div>
                    <div className="h-3.5 w-24 bg-slate-200 dark:bg-neutral-800 rounded"></div>
                  </div>
                </div>
                ...
                displayComponent.attributes.map((attr, idx) => (
                  <div
                    key={idx}
                    className="flex justify-between py-2 px-3 transition hover:bg-slate-100/50 dark:hover:bg-white/[0.01]"
                  >
                    <span className="font-semibold text-slate-500 dark:text-neutral-400">
                      {attr.name}
                    </span>
                    <span className="font-bold text-slate-700 dark:text-neutral-200 text-right">
                      {attr.value} {attr.unit || ''}
                    </span>
                  </div>
                ))
              ) : (
                <div className="py-4 text-center text-slate-400 dark:text-neutral-500 italic">
        ...
        {showAddToCart && (
          <div className="flex gap-3 pt-4 border-t border-slate-100 dark:border-neutral-900/80">
            <Button
              type="button"
              fullWidth
              onClick={() => onAddToCart?.(displayComponent)}
              disabled={displayComponent.status === 'agotado'}
              className="flex-1 bg-teal-500 hover:bg-teal-600 text-white font-bold h-10 transition duration-200"
            >
